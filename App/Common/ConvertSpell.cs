...
namespace App.Common {
	class ConvertSpell {
		#region 数组信息
		private static int[] arrChinese = new int[] {
		...
		private static string[] arrSpell = new string[] {
		...
		/// 二级汉字数组
		/// </summary>
		private static string[] arrSecondChinese = new string[] {
			...
			//补充生僻字
			"囧", "冏", "贠", "赟", "钘", "焜", "迺", "郞", "贇", "雲", "郤", "镠", "骉", "锧", "韡", "鐘", "鋆", "進", "趙"
		};

		/// <summary>
		/// 二级汉字对应拼音数组
		/// </summary>
		private static string[] arrSecondSpell = new string[] {
			...
			//补充生僻字
			"Jiong", "Jiong", "Yun", "Yun", "Xing", "Kun", "Nai", "Lang", "Yun", "Yun", "Xi", "Liu", "Biao", "Zhi", "Wei", "Zhong", "Yun", "Jin", "Zhao"
		};
		...
		private const int intFirstChCode = -20319;
		// GB2312-80 标准规范中最后一个汉字的机内码.即"齄"的机内码
		private const int intLastChCode = -2050;
		// GB2312-80 标准规范中最后一个一级汉字的机内码.即"座"的机内码
		private const int intLastOfOneLevelChCode = -10247;
		// 配置中文字符
		...
		public static String get(Char ch) {
			...
			if (chr > intLastChCode || chr < intFirstChCode) {
				return (ch.ToString());
			} else if (chr <= intLastOfOneLevelChCode) {
				// 如果是在一级汉字中
				// 将一级汉字分为12块,每块33个汉字.
				for (int aPos = 11; aPos >= 0; aPos--) {
					int aboutPos = aPos * 33;
					// 从最后的块开始扫描,如果机内码大于块的第一个机内码,说明在此块中
					if (chr >= arrChinese[aboutPos]) {
				...
				// 如果是在二级汉字中
				int pos = Array.IndexOf(arrSecondChinese, ch.ToString());
				if (pos != decimal.MinusOne) {
					return (arrSecondSpell[pos]);
				}
			}
			#endregion 中文字符处理
