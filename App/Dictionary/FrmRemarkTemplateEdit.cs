...
namespace App.Dictionary {
	public partial class FrmRemarkTemplateEdit : CCSkinMain {
		private int intId;
		//****************************************************************************************************
		public FrmRemarkTemplateEdit(int intId) {
			InitializeComponent();
		...
		//****************************************************************************************************
		//加载
		private void FrmRemarkTemplateEdit_Load(object sender, EventArgs e) {
			if (intId == 0) {
				this.Text = "添加" + this.Text;
			} else {
				this.Text = "修改" + this.Text;

				DataRow dr = new ModRemarkTemplate().setWhere("id > 0", true).getFind(intId);
				if (dr == null) {
		...
		private void btnSave_Click(object sender, EventArgs e) {
			...
			if (new ModRemarkTemplate().setSave(ht) == 0) {
