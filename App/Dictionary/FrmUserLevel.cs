...
namespace App.Dictionary {
	public partial class FrmUserLevel : CCSkinMain {
		//****************************************************************************************************
		//委托
		...
		private void showData() {
			DataTable dt = new ModUserLevel().setWhere("id > 0", true).getSelect();
			if (dt == null) {
		...
		private void btnInsert_Click(object sender, EventArgs e) {
			FrmUserLevelEdit frm = new FrmUserLevelEdit(0);
		...
		private void btnUpdate_Click(object sender, EventArgs e) {
			FrmUserLevelEdit frm = new FrmUserLevelEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
			if (frm.ShowDialog() == DialogResult.OK) {
				showData();
			}
		}
