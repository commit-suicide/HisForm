...
namespace App.Dictionary
{
    public partial class FrmInvoiceType : CCSkinMain
    {
        ...
        {
            DataTable db = new ModInvoiceType().setWhere("id > 0", true).getSelect();
            if (db == null)
            {
		...
		private void btnUpdate_Click(object sender, EventArgs e) {
			FrmInvoiceTypeEdit frm = new FrmInvoiceTypeEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
			if (frm.ShowDialog() == DialogResult.OK) {
				showData();
			}
		}
