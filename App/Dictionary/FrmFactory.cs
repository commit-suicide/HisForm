...
{
    public partial class FrmFactory : CCSkinMain
    {
        //***********************************************
        //委托
        ...
        {
            DataTable db = new ModFactory().setWhere("id > 0", true).getSelect();
            if (db == null)
            {
		...
		private void btnUpdate_Click(object sender, EventArgs e) {
			FrmFactoryEdit frm = new FrmFactoryEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
			if (frm.ShowDialog() == DialogResult.OK) {
				showData();
			}
		}
