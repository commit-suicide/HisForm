...
namespace App.Dictionary {
	public partial class FrmCardType : CCSkinMain {
		//****************************************************************************************************
		//委托
		...
		private void showData() {
			DataTable dt = new ModCardType().setWhere("id > 0", true).getSelect();
			if (dt == null) {
		...
		private void btnUpdate_Click(object sender, EventArgs e) {
			FrmCardTypeEdit frm = new FrmCardTypeEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
			if (frm.ShowDialog() == DialogResult.OK) {
				showData();
			}
		}
