...
namespace App.Dictionary
{
    public partial class FrmConformation : CCSkinMain
    {
        ...
        {
            DataTable db = new ModConformation().setWhere("id > 0", true).getSelect();
            if (db == null)
            {
        ...
        {
            FrmConformationEdit frm = new FrmConformationEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
            if (frm.ShowDialog() == DialogResult.OK)
            {
                showData();
            }
