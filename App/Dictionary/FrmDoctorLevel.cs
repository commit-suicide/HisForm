...
namespace App.Dictionary
{
    public partial class FrmDoctorLevel : CCSkinMain
    {
        ...
        {
            DataTable db = new ModDoctorLevel().setWhere("id > 0", true).getSelect();
            if (db == null)
            {
		...
		private void btnUpdate_Click(object sender, EventArgs e) {
			FrmDoctorLevelEdit frm = new FrmDoctorLevelEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
			if (frm.ShowDialog() == DialogResult.OK) {
				showData();
			}
		}
