...
namespace App.Dictionary
{
    public partial class FrmCure : CCSkinMain
    {
        ...
        {
            DataTable db = new ModCure().setWhere("id > 0", true).getSelect();
            if (db == null)
            {
        ...
        {
            FrmCureEdit frm = new FrmCureEdit();
        ...
        {
            FrmCureEdit frm = new FrmCureEdit(Convert.ToInt32(grd.CurrentRow.Cells["id"].Value));
            if (frm.ShowDialog() == DialogResult.OK)
            {
                showData();
            }
